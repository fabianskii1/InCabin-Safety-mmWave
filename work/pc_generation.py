...
# ---- numpy 호환성: 최신 numpy(>=1.24)에서 제거된 별칭 처리 ----
# np.complex / np.complex_ 는 구버전 전용. 아래로 안전하게 대체.
...
class PointCloudProcessCFG:  #
    def __init__(self):
        ...
        self.sumCouplingSignatureArray = np.zeros(
            (self.frameConfig.numTxAntennas, self.frameConfig.numRxAntennas,
...
def bin2np_frame(bin_frame):  #
    np_frame = np.zeros(shape=(len(bin_frame) // 2), dtype=_COMPLEX)
...
def naive_xyz(virtual_ant, num_tx=3, num_rx=4, fft_size=64):  #
    ...
    azimuth_ant_padded = np.zeros(shape=(fft_size, num_detected_obj), dtype=_COMPLEX)
    ...
    peak_1 = np.zeros_like(k_max, dtype=_COMPLEX)
    ...
    elevation_ant_padded = np.zeros(shape=(fft_size, num_detected_obj), dtype=_COMPLEX)
    ...
    peak_2 = np.zeros_like(elevation_max, dtype=_COMPLEX)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Usage: python pc_generation.py <adc_data.bin> <num_frames>')
        sys.exit(1)
    ...
    # bin 크기로 실제 frame 수 점검 (요청값이 파일보다 크면 자동 축소)
    frame_bytes = pointCloudProcessCFG.frameConfig.frameSize * 4  # int16 -> *2, 4채널 인터리브 *2
    file_bytes = os.path.getsize(bin_filename)
    max_frames = file_bytes // frame_bytes
    if total_frame_number > max_frames:
        print('[warn] requested %d frames but file has only %d. clamping.' % (total_frame_number, max_frames))
        total_frame_number = max_frames
    ...
    for frame_no in range(total_frame_number):
        ...
        if pointCloud.shape[0] == 0 or pointCloud.shape[1] == 0:  # 점이 없는 frame
            print('Frame %d: no points' % (frame_no))
            all_frames.append(np.zeros((128, 6), dtype=np.float32))
            continue

        raw_points = np.transpose(pointCloud, (1, 0))
        raw_points[:, :3] = raw_points[:, :3] + shift_arr
        # 점이 128개보다 많으면 무작위 128개 샘플, 적으면 무작위 중복으로 128개 채움
        raw_points = reg_data(raw_points, 128)
        print('Frame %d:' % (frame_no), raw_points.shape)
        all_frames.append(raw_points)

    bin_reader.close()

    # ---- 결과 저장 (원본에는 없던 부분) ----
    all_frames = np.array(all_frames)  # shape: (frames, 128, 6) -> [x,y,z,V,energy,R]
    out_path = 'pointcloud_output.npy'
    np.save(out_path, all_frames)
    print('Saved %s with shape %s' % (out_path, str(all_frames.shape)))
